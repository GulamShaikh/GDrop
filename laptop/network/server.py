...
def initialize_database() -> None:
    with sqlite3.connect(DB_PATH) as conn:
        ...
        # Table to persist gesture commands produced by the laptop gesture engine
        conn.execute(
            """
...
@app.post("/gesture-command")
def post_gesture_command(payload: dict):
    """Accept a gesture command POST from a trusted source and enqueue it for paired devices to poll.

    Expected JSON: { "name": "SEND_REQUEST" }
    """
    name = payload.get("name") if isinstance(payload, dict) else None
    if not name:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Command name is required.")

    now = utcnow().isoformat()
    with sqlite3.connect(DB_PATH) as conn:
        conn.execute(
            "INSERT INTO gesture_commands (name, created_at, processed_at) VALUES (?, ?, NULL)",
            (name, now),
        )
        conn.commit()

    return {"status": "ok", "name": name, "created_at": now}


@app.get("/gesture-command/poll")
def poll_gesture_command():
    """Return the oldest unprocessed gesture command, marking it processed.

    This is a simple polling endpoint for clients (e.g., Android) to fetch commands
    generated by the laptop gesture engine.
    """
    with sqlite3.connect(DB_PATH) as conn:
        row = conn.execute(
            "SELECT id, name, created_at FROM gesture_commands WHERE processed_at IS NULL ORDER BY id ASC LIMIT 1"
        ).fetchone()
        if row is None:
            # no command
            return HTMLResponse(status_code=status.HTTP_204_NO_CONTENT, content="")

        cmd_id, name, created_at = row
        processed_at = utcnow().isoformat()
        conn.execute(
            "UPDATE gesture_commands SET processed_at = ? WHERE id = ?",
            (processed_at, cmd_id),
        )
        conn.commit()

    return {"name": name, "timestamp": created_at}
